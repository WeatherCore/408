...
# -*- coding: utf-8 -*-
"""
pdfcraft — 纯 Python PDF 工具箱统一入口。

用法：
    python3 pdfcraft.py help                     # 列出所有命令
    python3 pdfcraft.py <command> help            # 查看命令详细帮助
    python3 pdfcraft.py <command> --arg value     # 执行命令

示例：
    python3 pdfcraft.py compress --input doc.pdf --output out.pdf --quality ebook
    python3 pdfcraft.py ir_export --input doc.pdf --output structure.json
"""
...
# 确保 pdfcraft.py 所在目录在 Python 路径中
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if _SCRIPT_DIR not in sys.path:
    sys.path.insert(0, _SCRIPT_DIR)

# 激活 venv（如果存在，与 pdfcraft.py 同目录下的 venv/）
...
def _get_extension_dir():
    """返回扩展脚本目录路径：与 skill basedir 同级的 pdfcraft-extension/scripts/

    例如 skill basedir 为 /path/to/pdfcraft/，则扩展目录为 /path/to/pdfcraft-extension/scripts/
    _SCRIPT_DIR = pdfcraft/scripts/  →  ../.. = pdfcraft 的父目录
    """
    return os.path.normpath(os.path.join(_SCRIPT_DIR, "..", "..", "pdfcraft-extension", "scripts"))


def _discover_extensions():
    """扫描 ../pdfcraft-extension/scripts/ 目录，收集扩展命令。
    ...
    for fname in sorted(os.listdir(ext_dir)):
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"pdfcraft_ext.{module_name}", fpath
            )
...
def _print_help(commands, extensions=None):
    ...
    print(f"pdfcraft — Pure Python PDF Toolkit ({total} built-in + {ext_count} extension commands)\n")
    ...
    print("Usage:")
    print("  python3 pdfcraft.py <command> --arg value")
    print("  python3 pdfcraft.py <command> help        # show command help")
    print("  python3 pdfcraft.py extension --help      # show extension commands")
...
def _print_command_help(mod, cmd_name):
    ...
    # 修改 prog 显示
    parser.prog = f"python3 pdfcraft.py {cmd_name}"
    parser.print_help()
...
def _print_extensions_help(extensions):
    """打印扩展命令的帮助信息。"""
    ext_dir = _get_extension_dir()
    print(f"pdfcraft extensions — User-defined PDF scripts\n")
    ...
    print("Usage:")
    print("  python3 pdfcraft.py extension --help                   # this help")
    print("  python3 pdfcraft.py <ext_command> help                 # extension command help")
    print("  python3 pdfcraft.py <ext_command> --arg value          # run extension command")
...
def main():
    ...
    if cmd_name == "extension":
        if len(sys.argv) >= 3 and sys.argv[2] not in ("help", "--help", "-h"):
            # pdfcraft.py extension <ext_cmd> ... → 转发到扩展命令
            cmd_name = sys.argv[2]
    ...
    if mod is None:
        ...
        print(f"Run 'python3 pdfcraft.py help' to see available commands.", file=sys.stderr)
        if not extensions:
            ext_dir = _get_extension_dir()
    ...
    # 剥掉 pdfcraft.py 和 command name，让 argparse 看到正确的参数
    sys.argv = [f"pdfcraft.py {actual_cmd}"] + sys.argv[2:]
